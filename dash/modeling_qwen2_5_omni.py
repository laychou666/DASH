...
class Qwen2_5OmniThinkerForConditionalGeneration(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def forward(
        ...
        # DASH: Dynamic Audio-Driven Semantic Chunking
        if pixel_values_videos is not None and audio_features is not None and attn_logits is not None:
            if self.dash_config is not None:
                dash_config = self.dash_config
            else:
                print("Using default DASH config")
                dash_config = {
                    "rho_audio": 0.3,
                    "rho_video": 0.6,
                    "g": 3,
            ...
            from dash.dash_units import dash
            inputs_embeds, global_mask, _ = dash(
                ...
                merging_ratio_audio=dash_config["rho_audio"],
                merging_ratio_v=dash_config["rho_video"],
                contextual_ratio=dash_config["contextual_ratio"],
                g=dash_config["g"],
                use_adaptive_grouping=dash_config.get("use_adaptive_grouping", True),
                audio_guidance_strength=dash_config.get("audio_guidance_strength", 0.3),
            )
